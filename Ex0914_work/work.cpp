...
#define DOWN 80
#define MAXSTAGE 3
#define putchxy(x,y,c) {gotoxy(x,y);_putch(c);}     //x, y자린에 c를 그린다 
...
void main()
{
	...
	for (; 1;) {
		...
		system("cls");                      //clear 해주는 함수   여기에는 왜?
		nMove = 0;
		 
		for (; ;) {                      //for(;1;) 무한루프
			DrawScreen();
			ch = _getch();                   //키보드 입력받고 출력안해줌 
			if (ch == 0xE0 || ch == 0) {     //확장키인지 확인(RIGHT, LEFT, UP, DOWN)
				ch = _getch();
				switch (ch) {
				case LEFT:
				case RIGHT:
	...
	if (ns[ny + dy][nx + dx] != '#') {
		if (ns[ny + dy][nx + dx] == 'O') {
			if (ns[ny + dy * 2][nx + dx * 2] == ' ' || ns[ny + dy * 2][nx + dx * 2] == '.') {
				...
				ns[ny + dy * 2][nx + dx * 2] = 'O';       //벽이나 아무것도 없으면 현재자리에서 왼쪽을눌렀을 때 2칸옆에 공이온다
			}
			else {
				return;
			}
